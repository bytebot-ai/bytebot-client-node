...
const PUPPETEER_ACT_TIMEOUT = 2000;
...
export class BytebotPuppeteer {
    ...
    public async act({
        ...
        page,
        options,
    }: BytebotPuppeteer.PuppeteerActOptions): Promise<Bytebot.ActResponseActionsItem[]> {
        ...
        this.logger.info(`Generated ${actions.length} action${actions.length > 1 ? "s" : ""}`);
    ...
    public async extract({
        ...
        page,
        options,
    }: BytebotPuppeteer.PuppeteerExtractOptions): Promise<[Bytebot.ExtractResponseAction] | []> {
        const { url, html } = await this.getPageData(page);
        this.logger.info("Generating actions");
        ...
        const actions: [Bytebot.ExtractResponseAction] | [] = response.action ? [response.action] : [];
        return actions;
    }
    ...
    public async execute({
        actions,
        page,
    }: BytebotPuppeteer.PuppeteerExecuteOptions): Promise<null | string | { [key: string]: string | null }[]> {
        this.logger.info(`Executing ${actions.length} action${actions.length > 1 ? "s" : ""}`);
    ...
    private isActResponseActionsItemArray(actions: any[]): actions is Bytebot.ActResponseActionsItem[] {
        if (actions.length === 0) {
            return false;
        }
    ...
    private isExtractResponseActionArray(actions: any[]): actions is [Bytebot.ExtractResponseAction] {
        if (actions.length != 1) {
            return false;
        }
    ...
    private resolveParameters(action: Bytebot.ActResponseActionsItem, parameters: { [key: string]: any }): void {
