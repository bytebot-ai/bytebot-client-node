...
import { AttributesType, BooleanAttributesType, attributeNames, booleanAttributeNames } from "./types/attributesType";
...
async function runEvaluateOneElement(
    ...
    return page.$x(xpath).then(async (elementNodes): Promise<Bytebot.ActionDetailResult> => {
        try {
            ...
            return elementNodes[0].evaluate((node) => evaluateFun(node as HTMLElement));
        } finally {
 ...
 */
export function copyText(actionOption: Bytebot.ActionDetail, page: Page): Promise<Bytebot.ActionDetailResult> {
    return runEvaluateOneElement(actionOption.xpath, page, (node) => node.innerText ?? null);
}
...
export async function copyAttribute(
    ...
    try {
        if (elementNodes.length > 1) {
            throw new Error(`More than one element found for xpath ${actionOption.xpath}`);
        }
        if (elementNodes.length === 0) {
    ...
    try {
        ...
        await elementNodes[0].evaluate(
            (node, attribute, value, attributeNames, booleanAttributeNames) => {
                ...
                if (!booleanAttributeNames.includes(attribute as BooleanAttributesType)) {
...
export async function extractTable(
    ...
    try {
                    ...
                    (column, element, ...rows) => {
                        ...
                        switch (column.actionType) {
                            ...
                            case "CopyAttribute":
                                ...
                                break;
                            default:
                                throw new Error(`ExtractTable: unknown action type ${column.actionType}`);
                        }
        ...
        columns.forEach((column) => column.column.forEach(async (element) => await element.dispose()));
    }
}
