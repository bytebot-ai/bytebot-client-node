 ...
 */
export async function copyText(action: Bytebot.BrowserAction.CopyText, page: Page): Promise<null | string> {
 ...
 * @returns null
 */
export function click(action: Bytebot.BrowserAction.Click, page: Page): Promise<void> {
    return page.$x(action.xpath).then(async (elementNodes) => {
        try {
            if (elementNodes.length > 1) {
                throw new BytebotMultipleElementsError(action.xpath);
...
export async function copyAttribute(
    ...
    try {
        ...
        if (!actionOption.attribute) throw new BytebotInvalidParametersError(actionOption.type);
 ...
 * @returns null
 */
export async function assignAttribute(action: Bytebot.BrowserAction.AssignAttribute, page: Page): Promise<void> {
    // Extract the element from the page according to the xpath
    ...
    try {
        ...
        // Validate the parameters
        if (!action.attribute) throw new BytebotInvalidParametersError(action.type, "attribute");
        parameter_attribute = action.attribute;
        if (!action.value) throw new BytebotInvalidParametersError(action.type, "value");
...
async function getTableCellHandle(cell: Bytebot.ExtractTableColumnBrowserAction, page: Page): Promise<ExtractCell> {
    ...
    if (cell.name === undefined || cell.name === null) throw new BytebotInvalidParametersError(action.type, "name");
    if (action.type !== "CopyText" && action.type !== "CopyAttribute")
        throw new BytebotInvalidParametersError("never", "type");
    if (action.type === "CopyAttribute" && (action.attribute === undefined || action.attribute === null))
...
export async function extractTable(
    ...
    if (!actionOption.rows) throw new BytebotInvalidParametersError(actionOption.type, "rows");
    ...
    try {
        ...
        rows = await concatenatePromises(
            actionOption.rows.map(
                async (row) => await concatenatePromises(row.map(async (column) => getTableCellHandle(column, page)))
            )
        );
    } catch (e: any) {
        rows.forEach(async (row) => row.forEach(async (column) => await column.cellElement.dispose()));
        throw e;
    }
